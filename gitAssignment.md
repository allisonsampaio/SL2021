...
```
Exibe branches disponíveis.
```

```
Altera para outra branch.
```

```
Exibe informações sobre commits.
```

2. Tente usar `git log --graph --all`. O que acontece?
```
Exibe uma árvore com branches e commits.
```
...
```
Adicionado método foo a classe A. 
Classe B removida.
```
